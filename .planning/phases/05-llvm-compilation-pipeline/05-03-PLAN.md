...
**handlers/compile.rs:** Thin handler following established pattern:
```rust
pub async fn compile_program(
    State(state): State<AppState>,
    Path(program_id): Path<i64>,
    Json(request): Json<CompileRequest>,
) -> Result<Json<ApiResponse<CompileResponse>>, ApiError> {
    let service = state.lock().await;
    // Active program guard (per Phase 4 pattern)
    service.check_active_program(ProgramId(program_id))?;
...
<verification>
- `cargo check -p lmlang-codegen` and `cargo check -p lmlang-server` both compile
...
- `cargo check` passes for both codegen and server crates
</success_criteria>
