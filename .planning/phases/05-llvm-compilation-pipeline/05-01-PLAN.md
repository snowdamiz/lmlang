...
must_haves:
  truths:
    ...
    - "Object files produced by LLVM can be linked into executables via system cc"
  artifacts:
...
- If `!debug_symbols`, pass `-Wl,-S` to strip debug symbols (macOS)
- Pass `-lSystem` on macOS for C runtime
