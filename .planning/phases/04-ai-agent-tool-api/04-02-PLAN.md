...
must_haves:
  ...
  artifacts:
    ...
    - path: "crates/lmlang-core/src/graph.rs"
      provides: "modify_compute_node_op method on ProgramGraph"
      contains: "modify_compute_node_op"
  key_links:
    ...
    - from: "crates/lmlang-server/src/service.rs"
      ...
      via: "imports VerifyScope and VerifyResponse from schema"
      pattern: "use crate::schema::verify"
---
...
<task type="auto">
  ...
  <action>
    ...
    2. Create `crates/lmlang-server/src/service.rs` with `ProgramService`:
       ...
       **Import VerifyScope and VerifyResponse from schema (defined in Plan 01):**
       ```rust
       use crate::schema::verify::{VerifyScope, VerifyResponse};
       ```
       These types are defined in `crates/lmlang-server/src/schema/verify.rs` (created by Plan 01, Wave 1). Do NOT redefine them here or in handlers -- import from schema.
       ...
       **Mutation methods (TOOL-01):**
       ...
       - `apply_mutation(graph: &mut ProgramGraph, mutation: &Mutation) -> Result<(CreatedEntity option, EditCommand), ApiError>`: applies a single Mutation to the given graph, returns created entity and the EditCommand for undo recording. Maps Mutation variants to ProgramGraph methods:
         ...
         - ModifyNode -> graph.modify_compute_node_op(id, new_op) -> returns old op for undo (uses the new method added below)
       ...
       **Validation method (TOOL-03):**
       - `verify(scope: VerifyScope, affected_nodes: Option<Vec<NodeId>>) -> Result<VerifyResponse, ApiError>`:
         ...
         - Return VerifyResponse { valid: bool, errors: Vec<DiagnosticError>, warnings: Vec<DiagnosticWarning> }.
    ...
    4. Add `modify_compute_node_op` to `crates/lmlang-core/src/graph.rs` as described above. This is a small addition of one method to ProgramGraph.

    5. Verify: `cargo check -p lmlang-server && cargo test -p lmlang-core`.
  </action>
  ...
  <done>ProgramService implements all 5 capabilities (mutate, query, verify, simulate, undo) with proper edit log recording, clone-and-swap batch atomicity, and checkpoint management. AppState wraps in Arc<Mutex>. ProgramGraph gains modify_compute_node_op method. VerifyScope and VerifyResponse imported from schema::verify (not redefined).</done>
</task>
...
- grep for `use crate::schema::verify` in service.rs confirms VerifyScope/VerifyResponse imported from schema
</verification>
...
- Verify supports local (affected nodes only) and full (entire graph) scopes using VerifyScope from schema::verify
