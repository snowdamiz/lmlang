...
must_haves:
  truths:
    - "Value, TraceEntry, RuntimeError, and TypeError all derive Serialize and Deserialize"
    - "lmlang-server crate exists in the workspace with all required dependencies"
    - "All API request/response types are defined with serde derives"
    - "API error type implements axum IntoResponse with structured JSON error format"
    - "Diagnostic types translate from internal TypeError/RuntimeError to API format"
  artifacts:
    - path: "crates/lmlang-server/Cargo.toml"
      provides: "Server crate manifest with axum 0.8, tokio, tower-http, serde, uuid, tracing"
...
<task type="auto">
  ...
  <done>serde derives added to Value, TraceEntry, RuntimeError, TypeError, FixSuggestion; lmlang-server crate exists in workspace and compiles</done>
</task>
...
<task type="auto">
  ...
  <action>
    ...
    2. Create `schema/mod.rs` re-exporting all sub-modules.
    ...
    10. Verify compilation: `cargo check -p lmlang-server`.
  </action>
  ...
  <done>All API request/response types defined, ApiError implements IntoResponse, From<TypeError> for DiagnosticError converts all variants, cargo check succeeds for both lmlang-check and lmlang-server</done>
</task>
...
<success_criteria>
- lmlang-server crate exists in workspace with axum 0.8, tokio, tower-http, serde, uuid, tracing dependencies
...
- Complete set of API schema types covers: mutations, queries, diagnostics, simulation, history, programs
