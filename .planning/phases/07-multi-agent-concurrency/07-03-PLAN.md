...
must_haves:
  ...
  artifacts:
    - path: "crates/lmlang-server/src/concurrency/verify.rs"
      provides: "run_incremental_verification for post-mutation verification"
      min_lines: 40
  ...
  key_links:
    - from: "crates/lmlang-server/src/concurrency/verify.rs"
      to: "lmlang-check::typecheck::validate_graph"
      via: "Type verification of modified subgraph"
      pattern: "validate_graph"
    - from: "crates/lmlang-server/src/concurrency/verify.rs"
      to: "lmlang-codegen::incremental::build_call_graph"
      via: "Call graph for transitive dependent discovery"
...
@crates/lmlang-check/src/typecheck.rs
</context>
...
<task type="auto">
  ...
  <action>
    **Create `concurrency/verify.rs`:**
    ...
    - Run `typecheck::validate_graph(graph)` on the full graph (the existing validate_graph is already efficient -- it checks all data edges).
    - Collect type errors as `Vec<DiagnosticError>`.
    - Return `VerifyResponse { valid, errors, warnings }`.

    Note: Full dirty tracking with `IncrementalState::compute_dirty` and `build_call_graph` from Phase 6 is used for *compilation* dirty detection, not type-checking. The type checker already validates all edges efficiently. Using it here keeps the verification path simple and correct. The `affected_functions` parameter is included for potential future optimization (scoped validation) but the initial implementation runs full validation since it is fast (Phase 6 validated 373-test programs in milliseconds).
