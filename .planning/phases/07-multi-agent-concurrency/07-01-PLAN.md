...
<task type="auto">
  ...
  <action>
    ...
    **Create `concurrency/lock_manager.rs`:**
    ...
    - `FunctionLockState` enum: `Unlocked`, `ReadLocked { readers: HashMap<AgentId, Instant>, expires_at: Instant }`, `WriteLocked { holder: LockHolderInfo, waiters: Vec<AgentId> }`. The `waiters` vec tracks agents that were denied the lock in FIFO order; when a new agent is denied, it is appended. When the lock is released, the waiters list is cleared (agents must re-request). This gives meaningful `queue_position` values in `LockDenial`.
    ...
    - `try_acquire_read(&self, agent_id: &AgentId, func_id: FunctionId) -> Result<LockGrant, LockError>`: If unlocked or ReadLocked, add reader. If WriteLocked by same agent, allow (downgrade read). If WriteLocked by another, append agent_id to waiters if not present, set queue_position in LockDenial to agent's 1-based index in waiters, then deny.
    - `try_acquire_write(&self, agent_id: &AgentId, func_id: FunctionId, description: Option<String>) -> Result<LockGrant, LockError>`: If unlocked, acquire (create WriteLocked with empty waiters). If ReadLocked only by this agent, upgrade to write. If WriteLocked by another: append agent_id to `waiters` if not already present, set `queue_position` in `LockDenial` to the agent's 1-based index in the waiters vec, then deny.
    ...
    - `release(&self, agent_id: &AgentId, func_id: FunctionId) -> Result<(), LockError>`: Remove agent from readers or release write lock. On write lock release, the waiters list is discarded (waiting agents must re-request). Remove entry from DashMap if unlocked.
