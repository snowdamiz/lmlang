...
4. Create `crates/lmlang-storage/src/traits.rs`:
   ...
   - **Incremental save design (per research conclusion, Open Question 3):** The individual CRUD methods (insert_node, update_node, insert_edge, etc.) ARE the incremental save mechanism. Each call writes exactly one row. `save_program` is the high-level convenience method for initial bulk save of a newly created program (or full overwrite). No dirty-tracking structure is needed -- auto-persist through CRUD methods satisfies the "write only changed data" decision.
   ...
   - High-level convenience: `save_program(&mut self, id: ProgramId, graph: &ProgramGraph) -> Result<()>` (bulk save/update for initial save or full overwrite)
   - High-level convenience: `save_function(&mut self, id: ProgramId, func_id: FunctionId, graph: &ProgramGraph) -> Result<()>` (saves/overwrites a single function's nodes, edges, and function definition from the given ProgramGraph)
   - Node CRUD (incremental save): `insert_node`, `get_node`, `update_node`, `delete_node` -- all take `ProgramId` + `NodeId` + `&ComputeNode` as appropriate
   ...
   - Query methods (per user decision): `find_nodes_by_owner(ProgramId, FunctionId)`, `find_edges_from(ProgramId, NodeId)`, `find_edges_to(ProgramId, NodeId)`, `find_functions_in_module(ProgramId, ModuleId)`, `find_nodes_by_type(ProgramId, TypeId) -> Result<Vec<(NodeId, ComputeNode)>>` (maps to user decision `find_by_type`), `list_functions(ProgramId) -> Result<Vec<(FunctionId, FunctionDef)>>` (maps to user decision `find_functions`)
...
2. Create `crates/lmlang-storage/src/memory.rs`:
   ...
   - Implement `GraphStore for InMemoryStore`:
     ...
     - `save_function`: extract nodes owned by func_id from ProgramGraph, delete existing nodes/edges for that function from StoredProgram, insert the fresh set, update the function definition
     - Query methods: filter in-memory (find_nodes_by_owner filters by owner field, find_nodes_by_type filters by type_id field, list_functions returns all stored functions, etc.)
   - Add tests:
